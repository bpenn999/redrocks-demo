...
const CONTENT = path.resolve(__dirname, '..', 'content', 'agency.json');
...
if (data.learn.some(l => l.slug === post.slug)) {
  console.error(`A post with slug "${post.slug}" already exists — skipping.`);
  process.exit(2);
}
data.learn.unshift(post); // newest first
fs.writeFileSync(CONTENT, JSON.stringify(data, null, 2) + '\n');
console.log(`✓ Added "${post.title}" (/learn/${post.slug}/). Now run: node site/engine/render.js`);
